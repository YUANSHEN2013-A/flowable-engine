...
public class MultiInstanceCandidateGroupsValidator extends ProcessLevelValidator {
    ...
    protected void executeValidation(BpmnModel bpmnModel, Process process, ProcessValidationContext validationContext) {
        ...
        for (UserTask userTask : userTasks) {
            ...
            if (StringUtils.isNotEmpty(elementVariable) && userTask.getCandidateGroups() != null) {
                for (String candidateGroup : userTask.getCandidateGroups()) {
                    if (StringUtils.isNotEmpty(candidateGroup) && !expressionReferencesVariable(candidateGroup, elementVariable)) {
                        validationContext.addError(Problems.CANDIDATE_GROUPS_NOT_USING_ELEMENT_VARIABLE, process, userTask,
                                "Candidate groups expression must reference the elementVariable '" + elementVariable + "'");
                    }
                }
            }
        }
