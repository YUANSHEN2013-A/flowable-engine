...
public class MultiInstanceAssigneeValidator extends ProcessLevelValidator {
    ...
    protected void executeValidation(BpmnModel bpmnModel, Process process, ProcessValidationContext validationContext) {
        ...
        for (UserTask userTask : userTasks) {
            ...
            if (StringUtils.isNotEmpty(elementVariable) && StringUtils.isNotEmpty(assignee)) {
                if (!expressionReferencesVariable(assignee, elementVariable)) {
                    validationContext.addError(Problems.ASSIGNEE_NOT_USING_ELEMENT_VARIABLE, process, userTask,
                            "Assignee expression must reference the elementVariable '" + elementVariable + "'");
                }
            }
        }
    }
